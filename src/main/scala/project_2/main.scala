...
object main{
  ...
  class hash_function(numBuckets_in: Long) extends Serializable {  // a 2-universal hash family, numBuckets_in is the numer of buckets
    ...
    val a: Long = (rand.nextLong %(p-1)) + 1  // random number is [1,p]
    val b: Long = (rand.nextLong % p) // random number in [0,p]
...
/* A constructor that requies intialize the bucket and the z value. The bucket size is the bucket size of the sketch. */
  ...
  def tidemark(x: RDD[String], trials: Int): Double = {
    ...
    val answer = x3.map(z => scala.math.pow(2,0.5 + z)).sortWith(_ < _)( trials/2) /* Take the median of the trials */
  ...
  def BJKST(x: RDD[String], width: Int, trials: Int) : Double = {
    ...
    val answer = x3.map(sketch => scala.math.pow(2,sketch.z.toDouble)*sketch.bucket.size).sortWith(_ < _)( trials/2)
    return answer
  }


  def Tug_of_War(x: RDD[String], width: Int, depth:Int) : Long = {
    val mean = new Array[Double](depth)
    var w = 0
      for (w<- 1 until depth) {
    ...
    return mean.sortWith(_ < _)(depth/2).toLong
  }


  def exact_F0(x: RDD[String]) : Long = {
    val answer = x.distinct.count
    return answer
  }


  def exact_F2(x: RDD[String]) : Long = {
    val answer = x.map(z => (z, 1.toLong)).reduceByKey(_ + _).map(z => z._2*z._2).reduce(_ + _);
    return answer
  ...
  //    val df = spark.read.format("csv").load("data/2014to2017.csv")
    ...
    if(args(1)=="BJKST") {
      ...
      val answer = BJKST(dfrdd, args(2).toInt, args(3).toInt)

      val endTimeMillis = System.currentTimeMillis()
      val durationSeconds = (endTimeMillis - startTimeMillis) / 1000

      println("==================================")
      println("BJKST Algorithm. Bucket Size:"+ args(2) + ". Trials:" + args(3) +". Time elapsed:" + durationSeconds + "s. Estimate: "+answer)
      println("==================================")
    }
      ...
      val answer = tidemark(dfrdd, args(2).toInt)
      val endTimeMillis = System.currentTimeMillis()
      val durationSeconds = (endTimeMillis - startTimeMillis) / 1000

      println("==================================")
      println("Tidemark Algorithm. Trials:" + args(2) + ". Time elapsed:" + durationSeconds + "s. Estimate: "+answer)
      println("==================================")
      ...
      val answer = Tug_of_War(dfrdd, args(2).toInt, args(3).toInt)
      val endTimeMillis = System.currentTimeMillis()
      val durationSeconds = (endTimeMillis - startTimeMillis) / 1000
      println("==================================")
      println("Tug-of-War F2 Approximation. Width :" +  args(2) + ". Depth: "+ args(3) + ". Time elapsed:" + durationSeconds + "s. Estimate: "+answer)
      println("==================================")
    }
      ...
      val answer = exact_F2(dfrdd)
      val endTimeMillis = System.currentTimeMillis()
      val durationSeconds = (endTimeMillis - startTimeMillis) / 1000

      println("==================================")
      println("Exact F2. Time elapsed:" + durationSeconds + "s. Estimate: "+answer)
      println("==================================")
    }
      ...
      val answer = exact_F0(dfrdd)
      val endTimeMillis = System.currentTimeMillis()
      val durationSeconds = (endTimeMillis - startTimeMillis) / 1000

      println("==================================")
      println("Exact F0. Time elapsed:" + durationSeconds + "s. Estimate: "+answer)
      println("==================================")
    }
    else {
      println("Usage: project_2 input_path option = {BJKST, tidemark, ToW, exactF2, exactF0} ")
